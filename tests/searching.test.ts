import { toSearchable, toSearchableMaximumTraversalDepth, makeMatchesExpressionPredicate } from './../src/arrays/searching';
import { getPeopleMmockData } from './people.mock-data';
import { search } from '../src/arrays/searching';
...
describe('search works with more simple expressions', () => {
  ...
  it('will not stringify too deep', () => {
    ...
    expect(searchable.split(/\s/).length).toBe(toSearchableMaximumTraversalDepth);
...
describe('makeMatchesExpressionPredicate', () => {
  it('has a default maximumDepth', () => {
    ...
    const explicitMaxDepthPredicate = makeMatchesExpressionPredicate('foo', toSearchableMaximumTraversalDepth);

    expect(autoMaxDepthPredicate.toString()).toEqual(explicitMaxDepthPredicate.toString());
  })
});